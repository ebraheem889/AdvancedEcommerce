                                            ...
                                            @isset($SubCategories)
                                                @foreach($SubCategories as $SubCategory)
                                                    <tr>
                                                        ...
                                                        <td>{{\App\Models\Category::Parentobject($SubCategory->parent_id)->name}}</td>
                                                        <td>{{$SubCategory->slug}}</td>
                                                        <td>{{$SubCategory->is_active}}</td>
                                                        <td>
                                                            <div class="btn-group " role="group"
                                                                 aria-label="Basic example">
                                                                <a href="{{route('admin.categories.edit',[$SubCategory->id ,'subcategory'])}}"
                                                                   class="btn btn-outline-primary btn-min-width box-shadow-3 mr-1 mb-1">تعديل</a>
                                                                <a href="{{route('admin.categories.destroy',[$SubCategory->id ,'subcategory'])}}"
                                                                   class="btn btn-outline-danger btn-min-width box-shadow-3 mr-1 mb-1">حذف</a>
                                                                <a href="{{route('admin.categories.changeStatus' ,[$SubCategory->id ,'subcategory'])}}"
                                                                   class="btn btn-outline-info btn-min-width box-shadow-3 mr-1 mb-1">

                                                                    @if($SubCategory-> is_active == 'مفعل')
                                                                        الغاء التفعيل
                                                                    @else
                                                                        تفعيل
                                                                    @endif
